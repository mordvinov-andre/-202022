...
float TOUCH_GOOD = 0;//maybe oshibka
...
static void homeaxis(const AxisEnum axis) {//Функция-сделана пустышкой, просто имитирует парковку в G28 Родион 14.11.20 
...
if (IsRunning()) {
   ...
   */
  inline void home_hangprinter() {//Имитация парковки, пустышка для работы G29
    homeaxis(X_AXIS);
    homeaxis(Y_AXIS);
    //homeaxisY(Y_AXIS);
    homeaxis(Z_AXIS);
...
inline void gcode_G92() {
  ...
  float X1 = ((457.20-planner.get_axis_position_mm(B_AXIS))/2.0),
        Z1 = 350.0-sqrt(sq(405.0)-sq(65.4)-sq((1034.0-(457.0-planner.get_axis_position_mm(B_AXIS)))/2.0-55.0-49.6));
    ...
    if (parser.seenval(axis_codes[i])) {
      ...
      if (!NEAR_ZERO(d)
         ...
         // if (i == Z_AXIS) current_position[Z_AXIS]=sqrt(sq(405.0)-sq(115.0-49.6)-sq(X2))-353.0;
           if(i != X_AXIS&& i != Z_AXIS) current_position[i] = v;      // Without workspaces revert to Marlin 1.0 behavior 
